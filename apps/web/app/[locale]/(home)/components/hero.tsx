...
export const Hero = async ({ dictionary }: HeroProps) => {
  const tHome = dictionary.web.home;
  const tHero = tHome.hero;           // for announcement / future hero-specific items
  const tGlobal = dictionary.web.global;

  return (
    <div className="w-full relative overflow-hidden">
      {/* Optional: keep GridPattern if it's part of your design – it's non-text */}
      <GridPattern
        width={40}
        height={40}
        x={-1}
        ...
        strokeDasharray={"4 2"}
        className={cn(
          "[mask-image:radial-gradient(ellipse_at_center,white,transparent_50%)] z-0"
        )}
      />
        ...
        <div className="flex flex-col items-center justify-center gap-8 py-16 sm:py-20 lg:py-40">
          ...
          <div className="flex flex-col items-center gap-4">
            <h1 className="max-w-4xl mx-auto text-center font-bold text-6xl sm:text-7xl md:text-8xl lg:text-9xl tracking-tighter leading-[0.9]">
              ...
              {/* SVG Fingerprint icon – kept as-is, title already accessible */}
            ...
            >
              ...
              <div className="relative inline-flex justify-center text-primary">
                <WordRotate />
              </div>
            </h2>
          ...
          <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 mt-4">
            ...
            >
              <Link href="/get-certified">
                {tGlobal.primaryCta || tHero?.ctaPrimary} <MoveRight className="h-4 w-4 sm:h-5 sm:w-5" />
              </Link>
            </Button>
            ...
            <Button
              asChild
              className="gap-2 sm:gap-4 w-full sm:w-auto border-2"
              size="lg"
              variant="outline"
            >
              <Link href="/about">
                {tGlobal.aboutUs || tHero?.ctaSecondary || "About Us"}
              </Link>
            </Button>
          </div>
        </div>
